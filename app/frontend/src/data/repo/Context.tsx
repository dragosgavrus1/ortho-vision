...
export const RepoProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  const fetchPatients = async () => {
    try {
      ...
      const base_URL = "https://ortho-vision-backend.fly.dev//patients";
  ...
  const addPatient = async (patient: Omit<Patient, "id">) => {
    try {
      const response = await fetch("https://ortho-vision-backend.fly.dev//patients", {
        method: "POST",
  ...
  const updatePatient = async (patient: Patient) => {
    try {
      ...
      const response = await fetch(
        `https://ortho-vision-backend.fly.dev//patients/${patient.id}`,
        {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify(patient),
  ...
  const getUserInfo = async (userId: number) => {
    const response = await fetch("https://ortho-vision-backend.fly.dev//get_user", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
  ...
  const updateUser = async (user: { id: string; fullname: string }) => {
    try {
      const response = await fetch(`https://ortho-vision-backend.fly.dev//users/${user.id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
